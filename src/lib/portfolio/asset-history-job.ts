...
export async function runAssetHistoryJob(userId: string, currentPairs: AssetPair[]): Promise<void> {
  ...
  try {
    ...
    const fetched = await ensureAssetSeries(userId, zerionKey, await loadTrackedPairs(userId))
