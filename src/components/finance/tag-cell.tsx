  ...
  return (
    <div ref={ref} className="relative inline-flex items-center gap-1 max-w-[170px]">
      {tags.slice(0, 2).map((t) => {
        ...
        return (
          <span key={t} className="inline-flex items-center gap-0.5 rounded-full border border-card-border text-foreground-muted text-[10px] font-medium px-1.5 py-0.5 max-w-[90px] truncate">
            <span className="material-symbols-rounded" style={{ fontSize: 10 }} aria-hidden="true">{m.icon}</span>{m.label}
          </span>
        )
      })}
      ...
      {open && (
        <div className="absolute top-full right-0 mt-1 z-50 w-64 bg-card border border-card-border rounded-xl shadow-xl p-2.5 animate-in fade-in slide-in-from-top-1 duration-150">
