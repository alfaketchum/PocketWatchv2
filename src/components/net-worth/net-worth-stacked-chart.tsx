...
export function NetWorthStackedChart({ data, height = 280, onDrill }: Props) {
  ...
  const points = useMemo(() => data.map((d) => ({ t: new Date(d.date).getTime(), d })), [data])
  ...
  const totals = points.map((p) => layers.reduce((s, l) => s + (p.d[l.key as StackKey] || 0), 0))
  ...
  const bands = layers.map((layer) => {
    ...
    cumulative = cumulative.map((c, i) => c + (points[i].d[layer.key as StackKey] || 0))
    ...
    <div ref={containerRef} style={{ width: "100%", position: "relative" }}>
      <svg width={width} height={height} onMouseMove={handleMove} onMouseLeave={() => setHoverIdx(null)} style={{ display: "block" }}>
        {yTicks.map((tk, i) => (
          <g key={i}>
            ...
            <text x={PAD.left - 8} y={tk.y + 3} textAnchor="end" fontSize={10} fontFamily="'IBM Plex Mono', monospace" fill="var(--foreground-muted)" opacity={0.7}>{fmtCompact(tk.val)}</text>
          </g>
        ))}
        {xTicks.map((tk, i) => (
        ...
        {bands.map((b) => (
          <g key={b.layer.key} onClick={() => onDrill?.(b.layer.key as StackKey)} style={{ cursor: onDrill ? "pointer" : "default" }}>
      ...
      {hp && (
        <div className="absolute pointer-events-none rounded-lg border border-card-border bg-card shadow-lg px-3 py-2 text-[11px] z-10"
          ...
          <div className="font-semibold text-foreground tabular-nums mb-1.5">{fmtFull(hoverTotal)}</div>
          {[...layers].reverse().map((l) => (
            (hp.d[l.key as StackKey] || 0) > 0.5 && (
              <div key={l.key} className="flex items-center gap-1.5 leading-tight">
                ...
                <span className="tabular-nums text-foreground">{fmtCompact(hp.d[l.key as StackKey])}</span>
              </div>
            )
          ))}
        </div>
